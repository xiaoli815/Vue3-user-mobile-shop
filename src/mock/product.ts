...
const productNamesByCategory: Record<number, string[]> = {
  1: ['iPhone 15 Pro Max', '华为 Mate 60 Pro', '小米14 Ultra', 'OPPO Find X7', 'vivo X100 Pro', '三星 Galaxy S24', '一加 12', '荣耀 Magic6 Pro', 'iPad Pro M4', 'MacBook Air M3', '华为 MatePad Pro', '联想小新 Pro 16', 'AirPods Pro 2', '华为 FreeBuds Pro 3', '小米手环 8 Pro', 'Apple Watch Ultra 2', '佳能 EOS R6 Mark II', '索尼 A7M4', '大疆 DJI Mini 4 Pro', 'GoPro Hero 12 Black', '任天堂 Switch OLED', 'PS5 Slim 光驱版', '罗技 G Pro X 机械键盘', '雷蛇 毒蝰 V3 Pro', '三星 T7 2TB 移动硬盘'],
  2: ['Nike Air Force 1', 'Adidas Ultraboost 轻量跑鞋', '李宁 超轻20 跑鞋', '安踏 C37+ 软底跑鞋', '优衣库 轻薄羽绒服', '波司登 极寒系列羽绒服', 'Levi\'s 501 经典牛仔裤', '海澜之家 商务休闲西裤', 'FILA 潮流卫衣', 'MLB 棒球帽', '蕉下 防晒衣', '骆驼 户外冲锋衣', 'New Balance 574 复古鞋', 'Under Armour 训练T恤', 'ZARA 女士风衣'],
  3: ['戴森 V15 吸尘器', '小米 米家扫拖机器人2', '美的 变频空调 1.5匹', '格力 云佳 冷暖空调', '海尔 十字对开门冰箱', '西门子 洗碗机 13套', '松下 电饭煲 SR-HQ153', '飞利浦 空气炸锅 HD9650', '九阳 破壁机 L18-Y928', '苏泊尔 电压力锅', '科沃斯 窗宝 W1S', '追觅 无线洗地机 H20', '戴森 空气净化器 HP09', '小米 净水器 1000G', '德龙 全自动咖啡机'],
  4: ['兰蔻 小黑瓶精华', '雅诗兰黛 小棕瓶眼霜', 'SK-II 神仙水 230ml', '海蓝之谜 面霜 60ml', '赫莲娜 绿宝瓶精华', '资生堂 红腰子精华', '完美日记 小细跟口红', '花西子 空气蜜粉', 'MAC 子弹头口红', '3CE 九宫格眼影盘', '欧莱雅 紫熨斗眼霜', '科颜氏 高保湿面霜', '倩碧 黄油乳液', '蒂佳婷 蓝药丸面膜', 'Tom Ford 乌木沉香香水'],
  5: ['三只松鼠 坚果大礼包', '良品铺子 肉脯礼盒', '来伊份 零食大礼包', '百草味 每日坚果', '蒙牛 特仑苏纯牛奶', '伊利 安慕希酸奶', '三顿半 精品速溶咖啡', '瑞幸 挂耳咖啡', '五粮液 52度 500ml', '茅台 飞天53度', '阳澄湖大闸蟹礼券', '海参 大连淡干礼盒', '稻香村 糕点礼盒', '元朗 蛋卷王', '德芙 巧克力礼盒'],
  6: ['宜家 毕利书架', '全友 真皮沙发', '顾家家居 科技布沙发', '林氏家居 北欧床 1.8m', '芝华仕 头等舱功能沙发', '慕思 乳胶床垫', '喜临门 独立弹簧床垫', '水星家纺 四件套', '富安娜 蚕丝被', '恒洁 智能马桶', '九牧 花洒套装', '欧普 吸顶灯', '公牛 轨道插座', '飞利浦 护眼台灯', '网易严选 人体工学椅'],
  7: ['花王 纸尿裤 L号', '贝亲 奶瓶 宽口径', '好孩子 婴儿推车', 'babycare 婴儿腰凳', '全棉时代 婴儿湿巾', '飞鹤 星飞帆 3段奶粉', '爱他美 卓萃 2段', '启赋 蕴淳 3段', '乐高 机械组 布加迪', '万代 高达模型 MG 自由', '泡泡玛特 盲盒', '迪士尼 玲娜贝儿玩偶', '小天才 电话手表 Z9', '牛听听 早教机', '逻辑狗 儿童思维训练'],
  8: ['《三体》全集', '《活着》', '《百年孤独》', '《人类简史》', '《原则》', '《小王子》', '《哈利·波特》全集', '《盗墓笔记》全集', '《新华字典》第12版', '《Python编程从入门到实践》', '《JavaScript高级程序设计》', '《深入理解Java虚拟机》', '晨光 按动中性笔 24支', '国誉 活页本 B5', '派克 钢笔 IM系列']
}
...
function productImage(name: string, categoryIdOrWidth: number = 1, height?: number) {
  // 判断第二个参数是分类ID还是宽度
...
for (let i = 1; i <= 30; i++) {
  ...
  const originalPrice = Math.round(price * faker.number.float({ min: 1.1, max: 2.5, fractionDigits: 1 }))
  ...
  productList.push({
    ...
    images: Array.from({ length: faker.number.int({ min: 3, max: 5 }) }, () => productImage(name, categoryId)),
    // 商品分类ID
    categoryId,
    // 商品销量（26-15800随机）
    ...
    flashSalePrice: i <= 4 ? Math.round(price * faker.number.float({ min: 0.7, max: 0.9, fractionDigits: 1 })) : 0,
    // 是否收藏（默认为false）
    isFavorite: false,
    // 商品标签（随机选取1-3个）
    tags: faker.helpers.arrayElements(['爆款', '新品', '限时特惠', '热卖', '品质优选', '店主推荐', '今日特价', '会员专享'], faker.number.int({ min: 1, max: 3 })),
    // 商品详情描述（3-6段随机文本）
